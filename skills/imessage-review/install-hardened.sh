...
# Cowork must be able to see the bridge. By default this is the selected
# folder into which bootstrap.sh copied this installer.
BRIDGE_ROOT="${CLAUDE_COWORK_IMESSAGE_BRIDGE:-$SOURCE_ROOT}"
PLIST_TEMPLATE="$SOURCE_ROOT/com.jeffhuber.claudecowork-imessage.plist.template"
...
LEGACY_WRAPPER="$BRIDGE_ROOT/bin/cowork-imessage-helper"
...
CURRENT_USER="$(id -un)"
BUILD_DIR="$(mktemp -d -t claude-cowork-imessage-build.XXXXXX)"
...
clang -Wall -Wextra -Werror -fobjc-arc \
    ...
    -o "$BUILD_DIR/claude-cowork-imessage-confirm" "$SOURCE_ROOT/bin/confirm_imessage_send.m"

clang -Wall -Wextra -Werror -O2 \
    -DHELPER_SCRIPT="\"$CODE_ROOT/bin/helper.py\"" \
    -DSEND_GATE_SCRIPT="\"$CODE_ROOT/bin/send_gate.py\"" \
    -DCONFIRM_HELPER="\"$CODE_ROOT/bin/claude-cowork-imessage-confirm\"" \
    ...
    -o "$BUILD_DIR/claude-cowork-imessage-helper" \
    "$SOURCE_ROOT/bin/imessage_helper.c"
...
codesign "${SIGN_ARGS[@]}" "$BUILD_DIR/claude-cowork-imessage-helper"
codesign "${SIGN_ARGS[@]}" "$BUILD_DIR/claude-cowork-imessage-confirm"
...
sudo /usr/bin/install -o root -g wheel -m 555 \
    "$BUILD_DIR/claude-cowork-imessage-helper" "$CODE_ROOT/bin/claude-cowork-imessage-helper"
sudo /usr/bin/install -o root -g wheel -m 555 \
    "$BUILD_DIR/claude-cowork-imessage-confirm" "$CODE_ROOT/bin/claude-cowork-imessage-confirm"
sudo /usr/bin/install -o root -g wheel -m 555 \
...
if [[ -e "$LEGACY_PLIST" || -L "$LEGACY_PLIST" ]]; then
    if "$PYTHON3_PATH" "$LEGACY_MIGRATOR" \
        ...
        rm -f "$LEGACY_PLIST"
        echo "  migrated this Claude install from legacy label $LEGACY_LABEL"
    else
...
Grant Full Disk Access to:
  $CODE_ROOT/bin/claude-cowork-imessage-helper
