#!/usr/bin/env python3
"""Diagnose a Claude Cowork iMessage helper installation without reading messages."""
...
def inspect_install(args: argparse.Namespace) -> dict[str, Any]:
    ...
    executable_files = {
        "fda_wrapper": code_root / "bin" / "claude-cowork-imessage-helper",
        "confirmation_helper": code_root / "bin" / "claude-cowork-imessage-confirm",
    }
    ...
    if not args.skip_launchd:
        ...
        if launchctl is None:
            ...
            uid = os.getuid()
            result = run([launchctl, "print", f"gui/{uid}/com.jeffhuber.claudecowork-imessage"])
