...
clang -Wall -Wextra -Werror -O2 \
  ...
  -DHELPER_DISPLAY_NAME='"test-helper"' \
  -fsyntax-only skills/imessage-review/bin/imessage_helper.c
...
if [ "$IS_MACOS" = true ]; then
  ...
  clang -Wall -Wextra -Werror -O2 \
    ...
    -DBRIDGE_ROOT='"/tmp/bridge"' \
    -DHELPER_DISPLAY_NAME='"test-helper"' \
    -o "$BAKED_HELPER" skills/imessage-review/bin/imessage_helper.c
...
if clang -Wall -Wextra -Werror -O2 \
  -DIMESSAGE_PRODUCT_BUILD=1 \
  -DHELPER_DISPLAY_NAME='"test-helper"' \
  -fsyntax-only skills/imessage-review/bin/imessage_helper.c 2>/dev/null; then
...
if [ "$IS_MACOS" = true ]; then
  ...
  clang -Wall -Wextra -Werror -O2 \
    "${PRODUCT_DEFINES[@]}" \
    -o "$TEST_HELPER" skills/imessage-review/bin/imessage_helper.c \
    "${PRODUCT_LINK_FLAGS[@]}"
...
clang -Wall -Wextra -Werror -O2 \
  ...
  skills/imessage-review/bin/imessage_helper.c \
  "${PRODUCT_LINK_FLAGS[@]}"
