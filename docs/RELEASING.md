...
1. Update `HELPER_VERSION` in `skills/imessage-review/bin/helper.py`, the
   versions in `skills/imessage-review/SKILL.md` and
   `.claude-plugin/plugin.json`, and `CHANGELOG.md` in the same pull request.
...
5. The Release workflow reruns the macOS checks, creates a draft release,
   uploads `.tar.gz` and `.zip` source archives, `imessage-review.plugin`, and
   `SHA256SUMS`, then publishes. Repository release immutability must be enabled
