...
ALLOWLIST_TOOL_PATH = SKILL_ROOT / "tools" / "configure_allowlist.py"
ALLOWLIST_SPEC = importlib.util.spec_from_file_location(
    "claude_configure_allowlist", ALLOWLIST_TOOL_PATH
)
...
"""
            ...
            result = subprocess.run(
                [sys.executable, "-c", probe, str(SKILL_ROOT / "bin" / "helper.py")],
                capture_output=True,
                text=True,
                check=False,
                env=env,
...
class WrapperValidationTests(unittest.TestCase):
    def test_product_path_derivations_use_checked_formatter(self) -> None:
        source = (SKILL_ROOT / "bin" / "imessage_helper.c").read_text()
...
class HardenedInstallerTests(unittest.TestCase):
    def test_installer_bakes_root_owned_code_and_allowlist_requirements(self) -> None:
        script = (SKILL_ROOT / "install-hardened.sh").read_text()
        ...
        self.assertIn('/Library/Application Support/ClaudeCoworkIMessage', script)
        self.assertNotIn("Grok", script)

    def test_runtime_privacy_files_are_gitignored(self) -> None:
        entries = (REPO_ROOT / ".gitignore").read_text().splitlines()
        self.assertIn("skills/imessage-review/contacts/allowed_chats.txt", entries)
        self.assertIn("skills/imessage-review/contacts/read_policy.txt", entries)

    def test_installers_preflight_existing_runtime_paths(self) -> None:
        for name in ("install.sh", "install-hardened.sh"):
            script = (SKILL_ROOT / name).read_text()
        ...
        self.assertIn('"$BRIDGE_ROOT/contacts/read_policy.txt"', hardened)
    ...
    def test_plist_separates_code_and_runtime_roots(self) -> None:
        ...
        self.assertIn("{{CODE_ROOT}}/bin/claude-cowork-imessage-helper", template)
    ...
    def test_allowlist_install_creates_tempfile_in_private_directory(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="claudecowork-allowlist-temp-test-") as td:
    ...
    def test_allowlist_install_works_with_root_owned_parent(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="claudecowork-hardened-test-") as td:
