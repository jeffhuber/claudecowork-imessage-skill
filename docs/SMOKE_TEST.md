...
This checklist verifies that the iMessage helper is installed correctly and working with your AI host (Claude Cowork or Grok Bot).
...
- [ ] Skill/plugin enabled in your AI host
...
## Test 4: AI Host Integration (Read)

**Ask your AI assistant to triage recent messages:**

- **Cowork:** "Triage my iMessages from the last day." or run `/imessage-review:imessages 1`
- **Grok Bot:** "Triage my iMessages from the last day."

**Expected:**
- The assistant writes a request file, polls for the response, and presents the triage (needs-reply / low-priority / skipped buckets).
- The assistant does **not** say "I can't access your iMessages" or "helper not found."

**If this fails:**
- **Cowork:** Verify the `.plugin` is installed and the bridge folder is selected as your workspace.
- **Grok Bot:** Verify you've provided the correct bridge folder path and the skill is enabled.
- Check `$BRIDGE/control/log.txt` for errors.
...
**Ask your AI assistant to preview a send:**

- **Cowork:** "Preview sending a message to +15551234567 that says 'Test message.'"
- **Grok Bot:** "Preview sending a message to +15551234567 that says 'Test message.'"

(Use a real phone number if you have one handy, or a fake one for this test—preview doesn't actually send.)

**Expected:**
- The assistant writes a `send_preview` request, polls for the response, and shows you:
...
- The assistant does **not** actually send anything yet.

**If this fails:**
- Check `$BRIDGE/control/log.txt` for errors.
- Verify the skill/plugin is correctly handling `send_preview` requests.
...
**Ask your AI assistant to send a message:**

- **Cowork:** "Text +15551234567: 'This is a test message from the iMessage helper.'"
- **Grok Bot:** "Text +15551234567: 'This is a test message from the iMessage helper.'"

**Expected:**
1. The assistant runs `send_preview` and shows you the preview.
2. The assistant waits for your explicit approval.
3. You confirm.
4. The assistant runs `send` with the `send_nonce` from the preview.
...
- **`send gate: missing nonce`:** The assistant didn't call `send_preview` first. This is a skill/plugin bug—report it.
- **`send payload differs from preview`:** The assistant changed the body or recipient between preview and send. Re-run the preview.
...
You can now use your AI assistant to:
...
| `send gate: missing nonce` | Skill/plugin didn't call `send_preview` first | Report a bug—the skill should always preview before send |
...
3. Open an issue at [github.com/jeffhuber/claudecowork-imessage-skill/issues](https://github.com/jeffhuber/claudecowork-imessage-skill/issues) with:
   - macOS version
   - AI host (Cowork or Grok Bot) and version
...
This smoke test is part of the `imessage-review` project, licensed under MIT.
