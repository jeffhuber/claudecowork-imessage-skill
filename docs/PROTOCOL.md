...
This document describes the JSON request/response protocol Claude Cowork uses
to communicate with the local macOS helper.
...
Claude Cowork can work with the user-selected bridge folder but cannot directly
open `~/Library/Messages/chat.db`. A macOS LaunchAgent bridges the two sides:
...
<code-root>/
└── bin/
    ├── claude-cowork-imessage-helper       # Compiled C wrapper (FDA target)
    ├── claude-cowork-imessage-confirm        # Native, fail-safe send confirmation
...
Required to send. First send triggers a one-time prompt: *"claude-cowork-imessage-helper wants to control Messages."* Click **OK**.
...
| `<code-root>/bin/claude-cowork-imessage-helper` | Locally signed C wrapper (the FDA target). |
