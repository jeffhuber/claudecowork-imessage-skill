...
This checklist verifies that the iMessage helper is installed correctly and working with Claude Cowork.
...
- [ ] Claude Cowork has access to execute shell commands on your Mac
...
```bash
launchctl list | grep com.jeffhuber.claudecowork-imessage
```

**Expected:** One line of output like:

```text
12345   0   com.jeffhuber.claudecowork-imessage
```
...
**If this fails:**
- Verify the plist exists: `ls -l ~/Library/LaunchAgents/com.jeffhuber.claudecowork-imessage.plist`
- Bootstrap manually: `launchctl bootstrap gui/$UID ~/Library/LaunchAgents/com.jeffhuber.claudecowork-imessage.plist`
...
## Test 4: Claude Cowork Integration (Read)

Confirm the `imessage-review` plugin is enabled in Claude Cowork. Then issue a
`status` request and confirm `protocol_version` begins with `1`,
`host_display_name` is `Claude Cowork`, and the installation checks are true.

**Ask Claude Cowork to triage recent messages:**

"Triage my iMessages from the last day."

**Expected:**
- Claude Cowork asks where your bridge folder is (if first time).
- Claude Cowork writes a request file, polls for the response, and presents the triage (needs-reply / low-priority / skipped buckets).
- Claude Cowork does **not** say "I can't access your iMessages" or "helper not found."

**If this fails:**
- Verify you've provided the correct bridge folder path to Claude Cowork.
- Check `$BRIDGE/control/log.txt` for errors.
...
**Ask Claude Cowork to preview a send:**
...
**Expected:**
- Claude Cowork writes a `send_preview` request, polls for the response, and shows you:
...
- Claude Cowork does **not** actually send anything yet.
...
**Ask Claude Cowork to send a message:**

"Text +15551234567: 'This is a test message from the iMessage helper.'"

**Expected:**
1. Claude Cowork runs `send_preview` and shows you the preview.
2. Claude Cowork waits for your explicit approval.
3. You confirm.
4. Claude Cowork runs `send` with the `send_nonce` from the preview.
...
**On first send only:** macOS will prompt: *"claude-cowork-imessage-helper wants to control Messages."* Click **OK**. This grants the Automation permission.

**If this fails:**
- **Automation prompt denied:** Re-enable in System Settings → Privacy & Security → Automation → claude-cowork-imessage-helper → Messages.
- **`send gate: missing nonce`:** Claude Cowork didn't call `send_preview` first. This is a skill bug—report it.
- **`send payload differs from preview`:** Claude Cowork changed the body or recipient between preview and send. Re-run the preview.
...
You can now use Claude Cowork to:
...
3. Open an issue at [github.com/jeffhuber/claudecowork-imessage-skill/issues](https://github.com/jeffhuber/claudecowork-imessage-skill/issues) with:
   - macOS version
   - Claude Cowork version (if applicable)
...
This smoke test is part of the `claudecowork-imessage-skill` project, licensed under MIT.
